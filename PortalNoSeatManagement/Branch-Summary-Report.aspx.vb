...
Partial Class Branch_Summary_Report
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            Try
                ...
                If String.IsNullOrEmpty(reqDate) Or String.IsNullOrEmpty(endDate) Or String.IsNullOrEmpty(branch) Then
                ...
                Else
                    'Date Defined
                    Try
                        Dim outletName As String = clsDataLayer.ReturnSingleValue("SELECT NAME FROM Branch WHERE ID=" + branch)
                        headerDate.Text = branch + ": " + reqDate + " - " + endDate
                        'No Error so lets continue
                        query = String.Format("SELECT su.PRODUCTS.FLD246 AS 'BusRoute',COUNT(DISTINCT(FLD123)) AS 'TotalPOS',COUNT(IDRELATION) AS 'TotalTickets',SUM(PRICE) AS 'TotalRevenue'" &
                        ...
                        ",SUM(CASE WHEN FLD118 ='RWF' THEN Total ELSE 0 END) as 'TotalRWF'" &
                            " FROM su.SALES_PROD INNER JOIN su.SALES  ON su.SALES_PROD.IDSALE = su.SALES.IDSALE INNER JOIN su.PRODUCTS ON su.SALES_PROD.IDPRODUCT=su.PRODUCTS.IDPRODUCT" &
                                    " WHERE  CAST(su.SALES_PROD.FLD108 AS DATE) BETWEEN CONVERT(DATE,'{0}',103) AND DATEADD(d, 1, CONVERT(DATE,'{1}',103)) AND FLD117 IN(SELECT ID FROM OUTLET WHERE BRANCH={2})" &
                                            " GROUP BY su.PRODUCTS.FLD246  ORDER BY 'TotalRevenue' DESC", reqDate, endDate, branch)

                        queryTotal = String.Format("SELECT SUM(TOTAL) AS 'Total','{0} - {1}'  AS 'TDATE',SUM(CASE WHEN FLD118 = 'RWF' THEN Total ELSE 0 END) as 'TotalRWF',SUM(CASE WHEN FLD118 ='FIB' THEN Total ELSE 0 END) as 'TotalFIB' FROM su.SALES_PROD WHERE CAST(su.SALES_PROD.DATE_MOD AS DATE) BETWEEN CONVERT(DATE,'{0}',103) AND DATEADD(d, 1, CONVERT(DATE,'{1}',103))  AND FLD117 IN(SELECT ID FROM OUTLET WHERE BRANCH={2}) ", reqDate, endDate, branch)
                        ...
                        queryTotalForDay = String.Format("SELECT SUM(TOTAL) AS 'Total', '{0} - {1}' AS 'TDATE' " &
                                                ...
                                                " FROM su.SALES_PROD WHERE CAST(su.SALES_PROD.FLD108 AS DATE) BETWEEN CONVERT(DATE,'{0}',103) AND DATEADD(d, 1, CONVERT(DATE,'{1}',103)) AND FLD117 IN(SELECT ID FROM OUTLET WHERE BRANCH={2})", reqDate, endDate, branch)
    ...
    Protected Sub tmrUpdateRecords_Tick(sender As Object, e As EventArgs) Handles tmrUpdateRecords.Tick
        Try
            ...
            Else
                ...
                queryTotal = String.Format("SELECT SUM(TOTAL) AS 'Total','{0} - {1}'  AS 'TDATE',SUM(CASE WHEN FLD118 = 'RWF' THEN Total ELSE 0 END) as 'TotalRWF',SUM(CASE WHEN FLD118 ='FIB' THEN Total ELSE 0 END) as 'TotalFIB' FROM su.SALES_PROD WHERE CAST(su.SALES_PROD.DATE_MOD AS DATE) BETWEEN CONVERT(DATE,'{0}',103) AND CONVERT(DATE,'{1}',103)AND FLD117 IN(SELECT ID FROM OUTLET WHERE BRANCH={2})", reqDate, endDate, branch)
                ...
                queryTotalForDay = String.Format("SELECT SUM(TOTAL) AS 'Total','{0} - {1}' AS 'TDATE' " &
                                        ...
                                        ",SUM(CASE WHEN FLD118 ='RWF' THEN Total ELSE 0 END) as 'TotalRWF'" &
                                            ...
                                            ",SUM(CASE WHEN FLD213 = 'PROMO' THEN 1 ELSE 0 END) as 'TotalPromo'" &
                                            ",SUM(CASE WHEN FLD213 <> '-' AND FLD213 <> 'PROMO' THEN 1 ELSE 0 END) as 'TotalBooking' " &
                                                 " FROM su.SALES_PROD WHERE CAST(su.SALES_PROD.FLD108 AS DATE) BETWEEN CONVERT(DATE,'{0}',103) AND CONVERT(DATE,'{1}',103)  AND FLD117 IN(SELECT ID FROM OUTLET WHERE BRANCH={2})", reqDate, endDate, branch)
        ...
        Try
            ...
            Else
                ...
                Try
                    ...
                    ",SUM(CASE WHEN FLD118 ='RWF' THEN Total ELSE 0 END) as 'TotalRWF'" &
                                " FROM su.SALES_PROD INNER JOIN su.SALES   ON su.SALES_PROD.IDSALE = su.SALES.IDSALE INNER JOIN su.PRODUCTS ON su.SALES_PROD.IDPRODUCT=su.PRODUCTS.IDPRODUCT" &
                                        " WHERE CAST(su.SALES_PROD.FLD108 AS DATE) BETWEEN CONVERT(DATE,'{0}',103) AND CONVERT(DATE,'{1}',103) AND FLD117 IN(SELECT ID FROM OUTLET WHERE BRANCH={2})" &
                                                " GROUP BY su.PRODUCTS.FLD246  ORDER BY 'TotalRevenue' DESC", reqDate, endDate, branch)
                    End If
