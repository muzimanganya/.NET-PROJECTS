                ...
                Else
                    ...
                    Try
                        ...
                        Else
                            ...
                            ",SUM(CASE WHEN FLD118 ='RWF' THEN Total ELSE 0 END) as 'TotalRWF'" &
                                " FROM su.SALES_PROD INNER JOIN su.SALES ON su.SALES_PROD.IDSALE = su.SALES.IDSALE" &
                                        " WHERE  CAST(su.SALES_PROD.FLD108 AS DATE) BETWEEN CONVERT(DATE,'{0}',103) AND DATEADD(d, 1, CONVERT(DATE,'{1}',103))" &
                                                " GROUP BY FLD120+'-'+FLD122  ORDER BY 'TotalRevenue' DESC", reqDate, endDate)

                            queryTotal = String.Format("SELECT SUM(TOTAL) AS 'Total','{0} - {1}'  AS 'TDATE',SUM(CASE WHEN FLD118 = 'RWF' THEN Total ELSE 0 END) as 'TotalRWF',SUM(CASE WHEN FLD118 ='FIB' THEN Total ELSE 0 END) as 'TotalFIB' FROM su.SALES_PROD WHERE CAST(su.SALES_PROD.DATE_MOD AS DATE) BETWEEN CONVERT(DATE,'{0}',103) AND DATEADD(d, 1, CONVERT(DATE,'{1}',103)) ", reqDate, endDate)
                            ...
                            queryTotalForDay = String.Format("SELECT SUM(TOTAL) AS 'Total', '{0} - {1}' AS 'TDATE' " &
                                                    ...
                                                    ",SUM(CASE WHEN FLD213 = 'PROMO' THEN 1 ELSE 0 END) as 'TotalPromo'" &
                                                    ",SUM(CASE WHEN FLD213 <> '-' AND FLD213 <> 'PROMO' THEN 1 ELSE 0 END) as 'TotalBooking' " &
                                                    " FROM su.SALES_PROD WHERE CAST(su.SALES_PROD.FLD108 AS DATE) BETWEEN CONVERT(DATE,'{0}',103) AND DATEADD(d, 1, CONVERT(DATE,'{1}',103))", reqDate, endDate)
                ...
                If String.IsNullOrEmpty(endDate) Then
                    ...
                    queryTotalForDay = String.Format("SELECT SUM(TOTAL) AS 'Total',CONVERT(VARCHAR(10),FLD108,105) AS 'TDATE' " &
                                        ...
                                        ",SUM(CASE WHEN FLD118 ='RWF' THEN Total ELSE 0 END) as 'TotalRWF'" &
                                           ...
                                           " AND MONTH(su.SALES_PROD.FLD108) = MONTH(CONVERT(DATETIME,'{0}',103)) AND YEAR(su.SALES_PROD.FLD108) = YEAR(CONVERT(DATETIME,'{0}',103)) GROUP BY CONVERT(VARCHAR(10),FLD108,105) ORDER BY 'TOTAL' DESC", exactDate)
                Else
                    queryTotal = String.Format("SELECT SUM(TOTAL) AS 'Total','{0} - {1}'  AS 'TDATE',SUM(CASE WHEN FLD118 = 'RWF' THEN Total ELSE 0 END) as 'TotalRWF',SUM(CASE WHEN FLD118 ='FIB' THEN Total ELSE 0 END) as 'TotalFIB' FROM su.SALES_PROD WHERE CAST(su.SALES_PROD.DATE_MOD AS DATE) BETWEEN CONVERT(DATE,'{0}',103) AND CONVERT(DATE,'{1}',103)", reqDate, endDate)
                    ...
                    queryTotalForDay = String.Format("SELECT SUM(TOTAL) AS 'Total','{0} - {1}' AS 'TDATE' " &
                                        ...
                                        ",SUM(CASE WHEN FLD118 ='RWF' THEN Total ELSE 0 END) as 'TotalRWF'" &
                                            ...
                                            ",SUM(CASE WHEN FLD213 = 'PROMO' THEN 1 ELSE 0 END) as 'TotalPromo'" &
                                            ",SUM(CASE WHEN FLD213 <> '-' AND FLD213 <> 'PROMO' THEN 1 ELSE 0 END) as 'TotalBooking' " &
                                                 " FROM su.SALES_PROD WHERE CAST(su.SALES_PROD.FLD108 AS DATE) BETWEEN CONVERT(DATE,'{0}',103) AND CONVERT(DATE,'{1}',103) ", reqDate, endDate)
        ...
        Try
            ...
            Else
                ...
                Try
                    ...
                    ",SUM(CASE WHEN FLD118 ='RWF' THEN Total ELSE 0 END) as 'TotalRWF'" &
                                " FROM su.SALES_PROD INNER JOIN su.SALES ON su.SALES_PROD.IDSALE = su.SALES.IDSALE" &
                                        " WHERE CAST(su.SALES_PROD.FLD108 AS DATE) BETWEEN CONVERT(DATE,'{0}',103) AND CONVERT(DATE,'{1}',103)" &
                                                " GROUP BY FLD120+'-'+FLD122  ORDER BY 'TotalRevenue' DESC", reqDate, endDate)
                    End If
